import { MousePointer2, Pen, Eraser, Type, Hand, ChevronDown, ChevronUp } from "lucide-react";
...
export default function CanvasToolbar({ 
    ...
    return (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 scale-90 origin-bottom z-30 flex flex-col items-center pointer-events-none">
            
            {/* Main Toolbar */}
            <div
                ref={toolbarRef}
                className={`ui-container bg-white/15 backdrop-blur-lg border border-white/50 shadow-lg rounded-lg px-4 py-2 flex items-center gap-3 max-w-[95vw] flex-wrap justify-center pointer-events-auto transition-all duration-300 ${isToolbarVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"}`}
            ...
            >
                {isViewer ? (
                    ...
                    <>
                        ...
                        <div className={`w-px h-8 ${isDark ? "bg-white/20" : "bg-base-300"} rounded-full`} />
                        <button 
                            className={`btn btn-sm btn-ghost border-none tooltip tooltip-top px-2`} 
