...
export default function MobileParticipantsStrip({ 
    isMobile, 
    participants, 
    followedUserId, 
    setFollowedUserId, 
    ...
    return (
        <div className="ui-container absolute top-5 left-5 mt-20 flex -space-x-3 z-50 pointer-events-auto">
            ...
            {participants.length > 5 && (
                <div className={`w-10 h-10 rounded-full border-2 border-white bg-base-300 text-base-content flex items-center justify-center text-xs font-bold shadow-sm`}>
                    +{participants.length - 5}
                </div>
            )}
        </div>
