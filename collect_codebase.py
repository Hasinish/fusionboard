...
# IMPORTANT: _JS_LINE_COMMENT only strips LEADING // lines, not inline //.
# Stripping all // would corrupt URLs in strings: "https://api.com" → "https:"
# For AI analysis quality we preserve inline comments; only pure comment lines removed.
_JS_BLOCK_COMMENT     = re.compile(r"/\*.*?\*/", re.DOTALL)
_JS_LINE_COMMENT_SAFE = re.compile(r"(?m)^[ \t]*//[^\n]*\n?")   # leading // only
...
def strip_comments(content: str, lang: str) -> str:
    """Remove stand-alone comment lines. Safe: never touches inline // in strings."""
    if lang in ("js", "ts", "java", "cs", "go", "rs", "kt", "swift",
                "cpp", "c", "php", "css", "scss", "sass"):
        content = _JS_BLOCK_COMMENT.sub("", content)
        content = _JS_LINE_COMMENT_SAFE.sub("", content)
    elif lang == "py":
        # Only inline # comments; preserve docstrings (they're API docs)
...
def is_generated(path: Path, first_lines: str = "") -> bool:
    ...
    for pat in GENERATED_PATTERNS:
        if pat.search(name) or pat.search(full):
            return True
    # Check first 3 lines for codegen marker comments
...
def read_file(
    ...
    # Binary detection: null bytes in first 8KB → skip
    if b"\x00" in raw_content[:8192].encode("utf-8", errors="replace"):
        return None
...
def find_generated_output_dirs(root: Path) -> set[Path]:
    """Find any dirs containing a collect_codebase manifest, at any depth."""
    result: set[Path] = set()
    # Only scan 2 levels deep to avoid traversing entire tree
    for d in root.iterdir():
        if not d.is_dir():
            continue
        for candidate in [d, *[x for x in d.iterdir() if x.is_dir()]]:
...
def collect_files(
    ...
    # Gather candidate paths using os.walk for ~18x speedup via early dir pruning
    ...
    for dirpath, dirs, files in os.walk(root, topdown=True):
        ...
        dirs[:] = sorted([
            ...
            and not any(
                (rel_dir / d).parts[:len(g.parts)] == g.parts
                for g in generated_dirs
            )
            and not (gitignore and gitignore.match_file(str(rel_dir / d) + "/"))
        ])

        for fname in sorted(files):
            abs_path = dp / fname
            rel      = rel_dir / fname
            # gitignore check on file
            if gitignore and gitignore.match_file(str(rel)):
                continue
            # Skip if inside a generated output dir
    ...
    # Sort: README first, then config → shared → backend → frontend → test → other
    CAT_ORDER = {"config": 0, "shared": 1, "backend": 2, "frontend": 3, "test": 4, "other": 5}
    def sort_key(e: FileEntry):
        # README always floats to top — it gives AI the best project overview first
        is_readme = e.rel_path.name.lower() in ("readme.md", "readme.txt", "readme.rst", "readme")
        return (0 if is_readme else 1, CAT_ORDER.get(e.category, 9), str(e.rel_path))
...
def build_chunk_header(
    ...
    if readme_summary and chunk_num == 1:
        # Inject README context only into first chunk — gives AI project narrative upfront
        header += f"#\n# PROJECT CONTEXT (from README):\n"
        for ln in readme_summary.splitlines()[:12]:   # first 12 lines max
            header += f"#   {ln}\n"
    header += f"#{'─'*76}\n"
    return header
...
def chunk_entries(entries: list[FileEntry], max_tokens: int) -> list[list[FileEntry]]:
    ...
    used = 0
    for e in entries:
        if e.token_count > effective:
            # Single file exceeds chunk limit — warn and place alone
            print(f"  ⚠  {e.rel_path}: {e.token_count:,} tok exceeds chunk limit "
...
def write_outputs(
    ...
    # Extract README summary for chunk header injection
    readme_summary = ""
    for e in entries:
        if e.rel_path.name.lower() in ("readme.md", "readme.txt", "readme.rst", "readme"):
            # Take first 12 non-empty lines of the actual README content
            lines = [ln for ln in e.content.splitlines() if ln.strip()][:12]
            readme_summary = "\n".join(lines)
            break
    ...
    manifest = {
        ...
        "chunks": [
            {"file": ("full_codebase.txt" if single else f"chunk_{i+1:02d}.txt"),
             "files": len(c), "tokens": sum(e.token_count for e in c)}
            for i, c in enumerate(chunks)
        ],
        "files": [
...
{'  2. full_codebase.txt' if single else chr(10).join(f'  {i+2}. chunk_{i+1:02d}.txt' for i in range(len(chunks)))}
""")
...
def main():
    ...
    entries = collect_files(root, out_dir, args.include_tests, args.strip_comments, gi)
